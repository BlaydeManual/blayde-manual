...
- **Fixed mobile text falling back to the browser's default 16px root on `index.html` and `docs/faq.html`.** Direct report: on the live site, the homepage's rings graphic itself read fine on a phone, but the surrounding text was hard to read next to it. Root cause: `maintainer.html`, `contribute.html`, and `registry-browse.html` all already carry `html { font-size: 17px; }` as their base rule, mobile included, from an earlier accessibility pass -- `index.html` and `docs/faq.html` never got it. `index.html` only ever had a *desktop* root-size rule (`clamp(18px, 1.4vw, 21px)`, gated behind a 900px+ media query), so it silently fell back to 16px on every phone; `docs/faq.html` had no root-size rule at all. Since nearly every size on both pages is a small `rem` fraction (0.78rem, 0.82rem, 0.85rem...) of whatever that root resolves to, the gap compounded into genuinely small mobile text across the board, not just one or two labels. Fixed by adding the same `html { font-size: 17px; }` base rule to both pages, placed before `index.html`'s existing desktop media query so that one still wins at 900px+ (same source-order rule already documented twice elsewhere in this file). Verified live: root resolves to 17px on both pages at a 375px viewport (was 16px), and still resolves to the existing desktop clamp (19.6px at 1400px) with no regression there.

- **Fixed erroneous clicks on the Community Tiers ring diagram, and let it grow on desktop.** Direct report: clicking one of the little scattered "population" dots on the homepage's rings diagram did nothing, no tooltip, no ring highlight, silently different from clicking the ring band a pixel away. Root cause: the dots (and the traveling arrival particles, and the pulsing "you are here" marker) are drawn after the clickable ring bands in the SVG's paint order, so they sit visually on top and, having no `pointer-events` of their own, silently absorbed the click instead of letting it fall through to the ring underneath -- SVG hit-testing follows paint order, not which element a person would say a dot "belongs to." Fixed by wrapping everything decorative in one `<g pointer-events="none">`, so a click anywhere in the diagram always reaches the real ring band beneath it; verified live by dispatching a click at a dot's exact screen coordinates and confirming `elementFromPoint` now resolves to the ring, not the dot. Separately, direct report that the diagram read as "less exciting" than the sections around it on a real desktop screen: it was capped at a flat 460px wide regardless of viewport, while the hero diagram and the "Pick your PDF manual" card above it both grow with the page. Grown to 720px (and the info panel below it to 560px) past a 900px breakpoint -- the viewBox is square, so `width:100%; height:auto` keeps it perfectly round at any size, no skewing. Caught and fixed a real mistake while building this: the desktop override was first placed before the base `.ct-rings-svg` rule in source order, which this same file already has a comment explaining doesn't work (equal-specificity CSS resolves by source order, not by which side is inside a media query) -- moved after, verified live at a 1400px viewport that the rings actually reach 720px, not silently staying at 460px. Direct follow-up report on that first version: growing the diagram made the problem worse, not better -- at 720px tall, selecting a ring pushed the info panel (still stacked below) out of view entirely, so seeing what a tier meant cost a scroll down and then a scroll back up to try another one, "scrolling back and forth isn't fun and ruins the graphic." Fixed by switching to a side-by-side layout at the same breakpoint (ring left, info panel right) instead of stacked, so selecting any tier never moves anything out of view; the connector line/arrow that used to bridge the vertical gap between a stacked diagram and panel no longer made sense once they're side by side (the selected ring's own pulse plus the panel's tier-colored border already say "this is about that ring"), so it's hidden at this breakpoint rather than re-deriving its path math for a layout it wasn't built for. Caught a second real bug while building this one: the ring's flex item defaulted to `min-width: auto`, which for a replaced element like an `<svg>` resolves to its own max-width and blocks shrinking below it -- confirmed live, this pinned the ring at 720px and pushed the panel into real horizontal overflow (not just a visual squeeze) at any width roughly between 900px and 1300px. Fixed with an explicit `min-width: 0` on the ring, verified live at 900px, 960px, and 1400px that the ring now shrinks smoothly to fit with zero horizontal overflow at any of them, still a perfect square at every size.
