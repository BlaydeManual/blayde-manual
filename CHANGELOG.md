...
- **Eliminated the personal fork from recategorization and manifest-fix proposals.** Direct follow-up to the wrong-session bugfix above: once that fix made both flows correctly prompt for a second, separate OAuth sign-in just to fork a repo, direct feedback flagged the real cost of that -- a personal fork sitting in someone's account forever, just to open one small proposal PR, on top of a second sign-in most people would never otherwise need. Asked directly why these two couldn't work like `/direct-contribute` (the Public photo path), which needs no fork at all: the App's own installation token already has write access to BlaydeManual's repos, so it can create a branch and open the PR directly. The one real reason it hadn't been built that way: `handleAcceptRecategorization`/`handleAcceptManifestChange`'s self-approval check (can't accept your own proposal) resolves the real proposer from `pr.user.login`, which is only meaningful because these PRs were fork-based -- if the App opened them instead, `pr.user.login` would always be the bot, silently breaking that check for everyone. Closed the same way `direct-contribute` already solves the equivalent problem for photos (attribution via the filename): added `POST /direct-recategorization` and `POST /direct-manifest-change` to the Worker, each creating its branch/PR directly with the App's installation token and setting the commit's author to the real proposer via a noreply email GitHub resolves back to that verified account. Added `resolveDirectProposer()` alongside the existing `resolveRealSubmitter()` to read that commit author back for the self-approval check in both accept handlers, leaving the photo-PR paths (`resolveRealSubmitter`'s four other call sites) completely untouched. `contribute.js`'s `submitRecategorizationProposal` and `issue-requests.js`'s `submitManifestChange` now just POST to their new endpoints instead of forking, and both click handlers dropped the Private-session gating the previous fix added, since neither needs it anymore -- back to the plain App session, same as every other everyday action on the site. The one fork-based flow that's staying exactly as it is: `submitPhotoPrivate`, where the fork isn't an implementation detail, it's the actual feature ("operate on your own," a deliberate personal copy held until the contributor chooses to propose it, or never). Updated SECURITY.md's and SECURITY-TESTING.md's trust-model writeups to match, including a note that Tiers 7.9/8.9 need fresh live verification against the new no-fork mechanism. Verified via `node --check` across all three changed files and by tracing the new endpoints line-for-line against `direct-contribute`'s already-proven pattern; not verified end-to-end against real GitHub OAuth (needs a live sign-in this environment can't drive).

- **Fixed two fork-based proposal flows always failing with "Resource not accessible by integration."** Direct report, live in the Contributor Portal: proposing a photo-location fix ("Page 173, procedure 4") failed immediately with that exact GitHub API error. Root cause: `issue-requests.js`'s `submitManifestChange` (the propose-a-fix flow) read `BlaydeAuth.getSession()`, the unified GitHub App session, then used its token to `POST /repos/{owner}/{repo}/forks`. `auth.js`'s own file-top comment already says why that can never work: "the App isn't installed on a fork that doesn't exist yet" -- forking into an arbitrary contributor's personal account is exactly the one case the classic OAuth `getPrivateSession()` exists for, scoped down to just that. Not a permission to grant, a wrong-session bug: no App installation-token scope covers this at all. Found the same exact bug, same shape, in `contribute.js`'s `submitRecategorizationProposal` (the "propose this vehicle's category change" flow) while checking for it elsewhere -- its own comment even says "same shape as contribute.js's submitRecategorizationProposal," which had silently carried the identical mistake. Fixed both to read `getPrivateSession()` instead, and updated both click handlers (`issueSubmitAllBtn`, `recatSubmitBtn`) to gate on and prompt for that specific session via `signInWithGitHubPrivate()` -- they used to gate on the App session, which is already live by the time either button is reachable, so neither ever actually prompted for the session the fork call needed. Matches the exact working pattern already proven in `contribute.js`'s own `submitPhotoPrivate`/`openPrForUpload` (the real Private-submit path), which reads `getPrivateSession()` correctly and was never affected. Checked every other `/forks` call in the codebase (three total) to confirm no other instance of this same bug exists. Verified via `node --check` on both files and by tracing the fix against the already-correct sibling function line for line; not verified end-to-end against real GitHub OAuth (needs a live sign-in this environment can't drive).

- **Softened pure white text on the FAQ page's dark accordion cards, direct follow-up to the background work above.** Same complaint, opposite side of the same cards: after the page background went from pure white to a real steel color, pure white heading/label text on the dark cards read just as rough by contrast. Added an `--off-white` token and applied it everywhere real heading or label text sits directly on a dark card background: section titles (`.acc-title`), diagram node names (`.big-node .name`), the approver/team headings and vehicle names in Who's In Charge, the "@YOU" credit tag text, and the Business Side badges and infinity glyph. First pass reused `.badge`'s existing hardcoded warm ivory (`#ede8dc`) -- direct, explicit rejection of that exact color as a hard rule going forward, not a preference. Landed on a cool off-white (`#e4e8ec`) instead, matching the same steel hue direction as `--bg`/`--steel`/`--steel-dark` rather than fighting it with a warm tone. Left small decorative glyphs alone (checkmark/plus badges on saturated red circles, not the near-black card background this was actually about) and the header's own white title/nav pill, which match the rest of the site's shared header treatment and weren't part of the report. Contrast headroom against every dark background this page uses is enormous either way (11:1-15:1, well past AAA), so this was purely a softness/hue choice, not a compliance fix like the background change was. Verified live: computed color matches `#e4e8ec` across every updated selector, no console errors. Direct follow-up right after: section headings specifically should "pop" rather than blend in ("it's an FAQ after all"). First attempt reused the page's existing muted gold (the Contribute tier's own color) on `.acc-title` -- direct rejection ("gross"). Rather than guess a third time, pitched four fresh, contrast-checked options (a brand coral-red, a vivid mint, a bold amber, and a cool azure, all 5.9:1-11:1 against the accordion's own dark background); bold amber (`#ffb454`, a `--heading-accent` token, 10:1) won. Applied to `.acc-title` only -- the other off-white spots from the previous entry (diagram labels, credit tag, badges) are staying put as label/body text, not headings. Direct follow-up on top of that: colored headings on a dark card can read a little flat without a defined edge, so pitched adding a thin dark halo -- reused `--red-halo` (`text-shadow: 0 0 3px/1px rgba(0,0,0,0.9)`), already used on this page's own logo text for exactly that purpose, rather than inventing a second halo value. Shown live across all five pitched colors at once (one per section heading) so the choice could be made with the halo already in place, not layered on after -- confirmed it read well on every candidate, not just the eventual winner. Amber won outright. Also added the same halo to `.approver-headline` ("2 community approvals per photo" and "Per-Manual Approvers"), the one other spot with light text sitting directly on a dark card background that got called out directly. Final state: `--heading-accent: #ffb454` with the halo on every section title, `--off-white: #e4e8ec` (no halo) on diagram/label text elsewhere, halo also on the approver headline text specifically.

- **Softened the FAQ page's pure white background to a real steel color, run through actual contrast math.** Direct report: white felt rough/harsh as a background, especially with every accordion card floating on it in its dark variant. Tinted `--bg` toward the same cool steel hue already used for `--steel`/`--steel-dark` rather than introducing a new, diverging color. Two rounds of direct follow-up ("barely better," then "still not enough, crank up the color") pushed this from a first pass (`#eef0f2`, barely off white) through a middle step (`#dde2e6`) to the final, clearly-colored `#c2ccd4`. Checked against WCAG contrast at every step, not just eyeballed: the actual limiting factor at this saturation is `--steel-dark`, used directly on this background as real text in four spots (`.expand-hint`, `.teaser-subtext`, `.acc-chev`, `footer`) -- it clears AA's 4.5:1 floor for normal text at 5.1:1, about as far as this hue can go before that specific text starts failing. `--text` (body's own default color, also sits directly on this background) has enormous headroom regardless, at 9.4:1. Caught and fixed a real pre-existing issue while checking this, not introduced by it: those same four spots used plain `--steel` before this work, which was already borderline-failing AA on the OLD white background (3.25:1) -- fixed by switching all four to `--steel-dark` (already defined, already used elsewhere as a border color, no new token needed). Verified live at each step: computed background color matches the current value, and the steel tone is now clearly visible, not a marginal shift.

- **Fixed erroneous clicks on the Community Tiers ring diagram, and let it grow on desktop.** Direct report: clicking one of the little scattered "population" dots on the homepage's rings diagram did nothing, no tooltip, no ring highlight, silently different from clicking the ring band a pixel away. Root cause: the dots (and the traveling arrival particles, and the pulsing "you are here" marker) are drawn after the clickable ring bands in the SVG's paint order, so they sit visually on top and, having no `pointer-events` of their own, silently absorbed the click instead of letting it fall through to the ring underneath -- SVG hit-testing follows paint order, not which element a person would say a dot "belongs to." Fixed by wrapping everything decorative in one `<g pointer-events="none">`, so a click anywhere in the diagram always reaches the real ring band beneath it; verified live by dispatching a click at a dot's exact screen coordinates and confirming `elementFromPoint` now resolves to the ring, not the dot. Separately, direct report that the diagram read as "less exciting" than the sections around it on a real desktop screen: it was capped at a flat 460px wide regardless of viewport, while the hero diagram and the "Pick your PDF manual" card above it both grow with the page. Grown to 720px (and the info panel below it to 560px) past a 900px breakpoint -- the viewBox is square, so `width:100%; height:auto` keeps it perfectly round at any size, no skewing. Caught and fixed a real mistake while building this: the desktop override was first placed before the base `.ct-rings-svg` rule in source order, which this same file already has a comment explaining doesn't work (equal-specificity CSS resolves by source order, not by which side is inside a media query) -- moved after, verified live at a 1400px viewport that the rings actually reach 720px, not silently staying at 460px.

- **Made the registry browse page's vehicle links actually do something, and tightened the mobile layout.** Direct report: browsing the registry, finding a manual, and downloading its source PDF from there dead-ended back on a blind, generic file picker, with no memory of which manual you'd just confirmed existed. Root cause: `registry-browse.js`'s own vehicle-title links already pointed at `index.html?vehicle=<slug>`, but that was a known dead link, called out in the file's own comment as "pointed at a per-vehicle page that doesn't exist." The real per-vehicle page was never the right fix (Blayde Manual never stores or serves the manual's own copyrighted PDF, so there's nothing to hand over there regardless), but the identification step didn't need to stay generic either: `patcher.js` now reads that same `?vehicle=` param, looks up the vehicle's display name in the registry, and scopes the file-pick prompt to it directly ("Continuing from the registry: Suzuki SV650 (1999). Drop the PDF you downloaded below."). If the PDF someone actually drops fingerprints to a different vehicle than the one they came from, a warning now surfaces in the log instead of silently proceeding, catching a wrong-download or wrong-edition mistake immediately rather than leaving it to be noticed later. Separately, fixed a real mobile layout gap: `body`'s flat 24px side padding, unchanged since desktop, read as a lot of wasted edge on a ~375px-wide phone screen with every card already stretching to fill it; a `max-width: 480px` rule now trims that to 10px, keeping just enough room for cards to still read as rounded rather than clipped. Verified live at a 375px viewport (screenshots before/after) and against two real registry slugs (a match and a deliberate miss) plus the plain no-param case, confirming the label only ever changes when the param is actually present.

- **Reworked the FAQ into a single-open accordion, plus one real bug found along the way.** `docs/faq.html`'s five sections (The Basics, Ways To Be Involved, Contributing, Who's In Charge, The Business Side) got a full content and structure pass: teaser diagrams that grow into the real diagram on expand and collapse on click, per-section subtext moved inline next to each heading (replacing repeated "expand for this" language with a single top-of-page instruction), tier names aligned to the current Use, Contribute, Maintain, Steer set, and every answer checked against its own section heading. That check surfaced two real mismatches, both fixed: added a missing card spelling out what each role actually looks like, color-coded per tier to match the rest of the page, and rewrote the Business Side card about telling manuals apart to describe the actual architecture decision behind it (structure comes from the manual itself, not a rigid database) instead of the money-focused framing it had inherited from an earlier section merge. Also fixed a real, separate bug found while reviewing the homepage's Community Tiers rings widget for consistency with the new FAQ wording: its connector-arrow dot (marks which ring is selected) had no fill and sat at (0,0) by default, waiting on a script to color and position it after page load. If that script's retry raced against layout still settling (fonts, images), the dot rendered as a solid black circle floating off the diagram instead of quietly doing nothing. Fixed by giving the dot `fill="none"` at rest, so a mistimed run stays invisible instead of visibly wrong, and by adding a `window` load listener alongside the existing retry so a late layout shift gets one more chance to correct it. Verified live in-browser: tag balance, both FAQ deep links from the homepage (`#how-this-works`, `#for-maintainers`) still open the right section, the role list renders in the right colors, and the connector dot is correctly colored on first paint with no console errors.

- **Made the All Manuals directory actually actionable, and added a small cross-portal link.** Direct follow-up: the directory could show a vehicle had zero real maintainers, but had no way to fix that from the same view -- the real invite/roster/join-request actions only ever lived in My Vehicles, which only shows repos the viewer already maintains. Same blind spot as the grant bug it was built to catch: the one place to act was invisible to whoever needed to act. Fixed by extracting My Vehicles' existing roster + join-requests + invite-by-handle card into a shared component (`renderVehicleMaintenanceCard`) and using it for every vehicle in the directory, not just ones the viewer already maintains -- same real, installation-token-backed actions either way, verified live against the real `royal-lexon-s20` shape (auto-expands with a clear warning, shows the real pending join request, offers a working invite). Also researched and theory-crafted combining the Contributor/Maintainer portals into one switchable app before deciding against it: real precedent (Shopify storefront/admin, WordPress site/wp-admin) keeps a similar majority-contributor/minority-maintainer split as genuinely separate surfaces, for reasons that hold here too -- different information needs, blast-radius separation, and not bloating the majority's experience with tooling they'll never use. Added the lighter version instead: a small secondary link between the two portals, near the sign-in status -- unconditional on the Maintainer Portal, shown on the Contributor Portal only for someone who actually has real maintainer access somewhere, so contributors (confirmed the large majority) never see chrome for a mode they have no reason to use. Sign-in itself needed no work either way -- already unified across both pages since the earlier one-sign-in pass.

- **Fixed a real maintainer-grant failure and added a directory to catch the next one.** Direct follow-up after checking who could actually maintain `royal-lexon-s20`: its real submitter had never received the automatic push-access grant every approved vehicle is supposed to give its requester -- the vehicle's only real collaborator was the org admin (structural access, not an intentional grant), and the person who'd actually asked for it had none at all, discovered only by manually auditing collaborator lists by hand. Root-caused to `handleApproveVehicle`'s grant call being a single best-effort attempt with no retry, silently swallowed on failure -- narrowed to a transient GitHub API failure (secondary rate limiting or a dropped connection are the documented candidates), though nothing was ever logged to say which for certain. Fixed at the source: the grant (and the two sibling calls that make the same request -- the new-edition grant, and the manual "request to maintain" invite) now retries with backoff on exactly the transient failure classes that can cause this, and a failure that survives every retry is reported back in the approval response instead of discarded. Direct pushback on a bigger version of this fix that would have only added detection, not reliability: "I feel this is chasing and putting process into place where a robust software solution solves it" -- correct, and the retry is what actually closes the gap; a one-time audit-and-alert would have needed someone to also know who to invite, information that only ever lived in a lookup nobody would think to do. Also added a member-visible "All Manuals" directory (Maintainer Portal, Approve New Manuals tab) as a generally useful thing to have regardless: every approved vehicle, grouped by category, sorted within each group by real maintainer count ascending -- a vehicle with zero real maintainers surfaces first as a natural byproduct of that sort, catching this exact case immediately going forward without anyone needing to go looking. Verified live: the retry logic against mocked transient/permanent/exhausted-retry scenarios, the directory's grouping/sorting/safeguard-row rendering against the real API response shape. Full detail in SECURITY.md.

- **Added a shared, reusable checker workflow so a future fix never needs hand-pushing to every vehicle repo again.** Direct prompt after fixing checker.py's resolution floor required pushing the same one-file change to four separate repos (see below): "what if it's thousands." `checker.py` and its workflow now live in exactly one place (`.github/workflows/validate-photo-reusable.yml`, this repo); a vehicle repo's own workflow becomes a one-line wrapper (`uses: BlaydeManual/blayde-manual/.github/workflows/validate-photo-reusable.yml@v1`), and updating the shared file propagates to every repo referencing that tag on its next PR, no per-repo edit ever again. Moving the tag itself stays a deliberate, gated action, not automatic on every merge -- one repo can be pointed at a canary ref to verify a change live before the shared tag moves for everyone else. Caught a real bug in testing before it ever reached a real vehicle repo: a first draft resolved the wrong context variable (`github.workflow_ref`, the CALLER's own ref) instead of the one actually scoped to the reusable workflow itself (`job.workflow_ref`) -- would have tried fetching `checker.py` from every calling repo's own ref instead of the pinned shared one, breaking on the very first real call. Verified live end-to-end (a real `workflow_call` boundary, real ref resolution, real cross-ref file fetch, confirmed correct content) before merging. `v1` tagged and every repo (`vehicle-scaffold` plus all three approved vehicle repos) converted to the thin-wrapper form, each own `checker.py` deleted -- confirmed live via each repo's own next real check run.

- **Fixed annotations silently never saving for any maintainer except the org's sole admin.** Direct report, confirmed via live testing across two real accounts: drawing annotations and clicking Approve in the Maintainer Portal left the manifest completely untouched, even with real 2/2 GitHub approvals in place -- `oem/manifest.json`'s entire git history had zero commits matching what that action should have produced. Root cause: the box/annotation commit that rides along with Approve (`persistReviewAdjustments`) wrote directly to the PR's base branch using the reviewer's OWN token -- which branch protection (2 required reviews, deliberately configured project-wide) silently rejects for anyone without admin/bypass rights. The org's sole admin never hit this (exempt by the existing `enforce_admins: false` setting), which is exactly why it went unnoticed until a second real maintainer tried it. Fixed the same way every other privileged write in this project already works: a new Worker endpoint verifies the caller is a real collaborator, then the App's installation token (which branch protection exempts by design) does the actual write. Verified against the real Worker/client code with a mocked network layer, including the failure path. Bonus finding, worth stating: the block itself was the required-review protection working exactly as designed, under a real non-admin account, not a gap in it -- the bug was a feature built on top without accounting for that, live confirmation the protection genuinely holds rather than something only asserted from config. See SECURITY.md.

- **Fixed a misleading PR-list status badge that made real, correctly-failing checks look like a stuck approval.** Direct report from live testing on `suzuki-sv650-1999` #17/#19: approving a PR and adding annotations left the badge reading "Waiting on others" even at 2/2 approved, with no indication anything was actually wrong. Investigated across every open PR on that repo -- 12 of 18 have a genuinely, permanently failing `checker` status check (the submitted photo's resolution is below the required 1200x900 minimum), which is GitHub correctly blocking merge, not a bug. The real bug: `review-panel.js`'s list-view badge (`prStatusInfo()`) never looked at the check-status data the Worker already returns, so once approvals were met it fell through to "Waiting on others"/"Needs your review" -- both wrong, since there's no one left to review and re-approving does nothing. The PR detail pane already surfaced this correctly (`updateAcceptButtonState()`, `renderReviewStatusLine()`); the list badge just never used the same data. Fixed by adding a `checksFailed` state that names the failing check(s) directly in the badge, matching the detail pane's existing `CHECK_LABELS` convention. Verified against the real function with PR #17's actual data shape. The underlying `checker` failure this surfaced is itself now fixed -- see the resolution-floor entry below.

- **Fixed checker.py's flat resolution floor rejecting genuinely high-resolution photos.** Root-caused directly: `suzuki-sv650-1999` #17/#19 both showed real 2/2 approvals stuck on a permanently-failing `checker` check. Traced the actual photos (real 6000x4000 Sony a6000 originals) through the full pipeline and found the real cause -- `contribute.js`'s own bbox-aware downscale (`computeTargetLongEdge`, see ROADMAP.md's "Repo-size math" entry) correctly, deliberately sized both photos down to ~1096px long edge for their small procedure bboxes, which a flat `1200x900` floor in `checker.py` then rejected regardless of how much real detail the source photo actually had. Fixed by making the floor bbox-aware, using the same manifest data `checker.py` already receives -- deliberately NOT an exact mirror of `contribute.js`'s formula (half its DPI target, none of its 2x headroom), so the two independent implementations don't need to stay in exact lockstep to keep working correctly. Verified against the real manifest and real source photos: the resolution check now passes at the actual submitted dimensions, and a synthetic large-bbox/small-photo case still correctly fails. `checker.py` isn't shared/synced across repos, so this needed pushing to `vehicle-scaffold` (the template) plus all three approved vehicle repos directly, not just this tooling repo.
...
- **One sign-in, everywhere.** Direct report: "I feel we have to authorize the app multiple times rather than one sign in," with two confirmed instances (the indexer's submit button, the Approve New Vehicles tab) both showing "signed in" at the top of the page and "sign in to do this" one section down -- both real, both the two-session design working exactly as built, not a bug in the strict sense, but a genuine UX problem (see ROADMAP.md's now-closed "sign-in state isn't consolidated" entry for the full root-cause history). Fixed by making the GitHub App session the one everyday sign-in for browsing, reviewing, approving, maintaining, and the contribute flow's Public submit -- removing the indexer's and Approve New Vehicles' own separate App-session gates entirely, since the whole Maintainer Portal is already unreachable without a session by the time either tab is visible. The classic OAuth App still exists, but scoped down to exactly one contextual action: the contribute flow's Private path (forking into the contributor's own account), which genuinely needs its own broader grant and now only ever prompts for it at the moment someone picks Private, never upfront. Every portal page now carries exactly one sign-in control, in the top-right nav, replacing the scattered per-page cards and buttons that used to gate different sections. Session storage also moved from `sessionStorage` to `localStorage` (survives new tabs and reopening the browser, not just reloading), and the Worker gained a `/app-token/refresh` endpoint so a stale access token can be silently traded for a fresh one in the background -- shipped alongside the XSS fixes above and with the GitHub App's own token-expiry setting turned on, closing the gap that combination would otherwise have left open. See SECURITY.md for the full reasoning.
