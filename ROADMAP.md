...
- **Should a reviewer be able to correct a wrong source URL without rejecting the whole submission?** The pane shows "Submitter's source" (`entry.manifest.source_markers?.source_identifier`, rendered read-only in `org-approval.js` around line 120) -- today there is no edit control for it. The real tension, checked directly rather than assumed: `source_identifier` lives INSIDE `manifest.source_markers`, which is part of the exact manifest content that gets sha256-hashed and notarized at `/direct-submit` time (see the GitHub App migration entry above). `handleApproveVehicle`'s notarization check (just live-verified working correctly via 4.5 above) recomputes that hash and rejects on ANY difference from the notarized original -- including a one-field URL fix. So "let the reviewer patch a typo" as currently architected would mean either (a) breaking the notarization guarantee for this one field (carving out an exception undermines the property that makes approval trustworthy at all), or (b) accepting that a wrong-but-otherwise-good submission has to be rejected and resubmitted rather than lightly corrected. No REJECT action is even built yet either (see SECURITY.md's "deliberately not built" note), which makes this sharper, not just theoretical -- resubmission today has no clean path even after a reject decision. Real open question, not decided; needs its own think-through rather than a quick fix.
...
## Fix: "My Vehicles" didn't show a repo the reviewer just approved (2026-08-27)

Direct report, right after approving `blayde-manual-2026` live: expected it to show under My Vehicles, suspected it wasn't wired up yet. Checked the actual code and live GitHub API responses rather than assuming -- it's a real, confirmed bug, not a missing feature.

**Root cause**: `discoverMaintainedRepos()` (`maintainer-portal.js`) queried `GET /user/repos?affiliation=collaborator` only. Confirmed live with the real account: this returns ZERO BlaydeManual repos for an org owner with full admin access on every one of them, because that access comes from the org role itself, not an explicit per-repo collaborator grant -- `affiliation=collaborator` specifically excludes that case. Re-ran with `affiliation=organization_member` and got all six real BlaydeManual repos back, correctly. The existing `permissions?.push` filter downstream was never the problem and needed no change -- it already correctly excludes a plain member with only the org's default read access.

**Fix**: changed the query to `affiliation=collaborator,organization_member`, covering both real ways someone can have write access to a repo (an outside collaborator invited without joining the org, and a real org member/owner whose access comes from their role). Verified live against the real API with the real account before writing the fix, not just after.

Scoped as its own PR (stacked on the still-open collaborator-management PR, since `discoverMaintainedRepos()` doesn't exist on `main` yet) per direct instruction.
