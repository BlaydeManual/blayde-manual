...
- **Expire user access tokens: still off, now for a different reason.** As of the one-sign-in pass (2026-09-11, see CHANGELOG.md), this classic OAuth App only handles the contribute flow's rare Private-fork path -- everyday sign-in moved to the GitHub App, which now has its own refresh-token handling in the Worker (`/app-token/refresh`). Building the same refresh logic for classic OAuth too isn't worth it for how rarely that path fires; this deferral is deliberate and expected to stay, not a gap still waiting to be closed.
