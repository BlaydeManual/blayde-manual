...
2. **A real "MyVersion" personal patch mode** -- sign in, patch using photos pulled from your OWN fork/repo instead of (or blended with) the canonical registry, with the missing-photo QR markers staying intact for anything not yet covered even in your personal copy. A materially bigger lift than (1): `patcher.js`'s registry mode is built around one canonical registry URL and one canonical per-vehicle repo; sourcing photos from a contributor's own fork instead means a real second data path through the same patch logic, not a flag. Buildable later, but a different shape of work than anything in the current codebase -- not scoped further than this until it's a real ask.
