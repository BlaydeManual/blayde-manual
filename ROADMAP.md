...
**Status, 2026-08-30: built and shipped across three PRs.** The "one repo per vehicle, editions as subdirectories" model (resolved 2026-08-25, above) is now real in every code path that was flagged as a gap when this section was first written:
- `scaffold/` (PR #55, Phase 1): `images/` moved under `{{EDITION_ID}}/images/`; `CONTRIBUTING.md`, `README.md`, the PR template, and both CI workflows (`validate-photo.yml`, `validate-manifest.yml`) all edition-aware. `checker.py`/`validate_manifest.py` needed no changes -- already path-agnostic via CLI args.
- `web/registry.js`, `web/patcher.js`, `web/contribute.js`, `web/registry-browse.js`, `web/issue-requests.js`, `web/review-panel.js` (PR #55, Phase 2): every manifest/photo path now threads `edition_id` through (`${editionId}/manifest.json`, `${editionId}/images/...`); `registry.json` rows are matched on `(repo_url, edition_id)` together, since multiple edition-rows can now legitimately share one `repo_url`.
- `auth-worker/src/index.js`, `web/org-approval.js` (PR #56, Phase 3): `handleDirectSubmit` detects an already-approved vehicle and routes a new edition into a disposable staging repo tagged `target_repo_url`; `handleApproveVehicle` branches on that tag to either flip a brand-new vehicle's staging repo public (unchanged) or copy the new edition's files into the existing target repo, grant collaborator access there, and delete the staging repo. `editionId` is always derived from the repo's own real directory structure, never trusted from the request body. The file-allowlist check walks the full recursive git tree (not two shallow `contents/` calls), so it verifies the whole repo is exactly `{README.md, <edition>/manifest.json}` with nothing else at any depth.

**A real deployment gap, found 2026-08-30 by asking "do existing repos need to match this":** shipping the code (PRs #55/#56) was not the same as the live system actually running on the new model. Four things were still wrong in production, all fixed via the standard branch+PR pattern:
- The live `BlaydeManual/vehicle-scaffold` template repo (what gets copied onto every newly-approved vehicle) was never updated — only this monorepo's local `scaffold/` copy was. Every approval since PR #55 merged silently applied the old flat scaffold. Fixed in `vehicle-scaffold` PR #3.
- The only two real, live vehicle repos (`suzuki-sv650-1999`, `blayde-manual-2026`) both predated the model and were still flat. Migrated in their own PRs (#2 and #10 respectively) to `oem/manifest.json` + `oem/images/`.
- `registry.json` had `edition_id: "OEM"` (uppercase) for both, which the new validation (`[a-z0-9-]` only) rejects and `registry.js` matches case-sensitively against the folder name — normalized to lowercase in `registry` PR #1, timed to land with the two migrations above.
- A real external contributor's pending photo PR on `suzuki-sv650-1999` was invisible in the Maintainer Portal because `review-panel.js`'s edition-path regex doesn't match a pre-migration flat path — not fixed with a compat shim (declined, per the project owner: "we can correct... if I need to override manually I will"), just sequenced to merge before that repo's migration.

Below is the original design note this status check was written against, kept for context on the reasoning, not as an open task list.
...
1. **Negative file-allowlist**: a direct-submit repo must contain EXACTLY `{README.md, manifest.json}` and exactly one branch. Anything else (most dangerously a `.github/workflows/*.yml`, which would execute with the org's own permissions the moment the repo goes public) is a hard, automatic block -- not a warning a human can click through. (Superseded 2026-08-30 by the edition-subdirectory work below: the shape is now `{README.md, <edition_id>/manifest.json}`, checked via a full recursive tree rather than two shallow levels -- see "Edition-subdirectory implementation," and SECURITY.md for the current description.)
...
**Fixed, 2026-08-27.** All three items above, across both `indexer-review.js` and `org-approval.js`: Prev/Next moved above the gallery in both (`maintainer.html`'s `#reviewPager`/`#orgPager`), and both submit actions (`submitBtn`, `orgApproveBtn`) now hide the whole review pane and show a real summary card on success, instead of a message sitting at the bottom of a long gallery. Caught two real follow-on bugs while building this, both fixed before shipping: (1) a leftover reset line in `startReview()` immediately re-hid the review section right after showing it -- caught by a synthetic DOM test, not by eyeballing the diff; (2) `org-approval.js`'s summary swap used an inline `style.display = "none"`, which outranks the `.open` CSS class `#orgReviewArea` normally uses for visibility -- without an explicit reset, a second pending vehicle would stay stuck hidden after the first one was approved. Both verified live via direct DOM state checks simulating the full open -> submit/approve -> reopen cycle.

**Three more findings from reviewing the real `blayde-manual-2026` My Vehicles/Issue Requests panes, not fixed in this pass:**
1. **The per-maintainer activity metrics from the old mocked roster (requests reviewed, last active) are gone** in the real `my-vehicles.js` roster, by design -- GitHub's collaborator API doesn't expose them, and computing them for real would mean iterating every merged PR per collaborator. Flagged directly, called "not huge" -- logged as a known, acknowledged simplification, not a regression to silently fix.
2. **My Vehicles shows "Covers 1 edition: OEM" for `blayde-manual-2026`; Issue Requests shows "(edition not set)" for the same repo.** Root cause confirmed: this is the ALREADY-documented, explicitly out-of-scope `issue-requests.js` mock-registry problem (`MOCK_REGISTRY.vehicles`, which doesn't contain `blayde-manual-2026` at all, versus My Vehicles' real registry.json read) -- now visibly surfacing in real use rather than just a known gap on paper.
3. **Issue Requests' own page viewer has no Prev/Next at all** -- only a "jump to page" input, a third, different navigation pattern from the two galleries just made consistent with each other. Logged as part of the same "make the editors consistent" ask, but not fixed here -- it's downstream of item 2's mock-registry problem, which needs solving first for this view to have real data to page through in the first place.
...
## Correction: "My Vehicles" org-role fix was the wrong mechanism (2026-08-27)

A same-day fix (adding `organization_member` to `discoverMaintainedRepos()`'s affiliation query, so an org owner's implicit access would surface a just-approved repo) was caught as wrong before merging, by direct pushback: "I don't want to see EVERYTHING as an org owner, that's not my role." Correct -- that fix conflated org role with maintainership, directly contradicting SECURITY.md's own stated principle that these are separate designations. Reverted.

**The real fix, precisely stated**: "since TheBlayde submitted the demo manual, shouldn't I see it because I was made a maintainer as part of the flow?" -- yes, and that's the actual gap: nothing in `handleApproveVehicle` ever granted the submitter real collaborator access to their own newly-public repo. Fixed by adding a real `PUT .../collaborators/{login}` grant (permission: `push`) at APPROVAL time, using `logEntry.github_login` (already available from the notarization check, a real verified identity from submit time, not user input). Deliberately at approval, never at submit -- granting it earlier would undo the entire point of the locked-repo design. Deliberately the submitter only, not the approving admin -- approving a vehicle doesn't make the approver its maintainer, matching the same separation principle. Best-effort (a failure here doesn't undo an otherwise-valid approval; a maintainer can always be added manually via My Vehicles' own invite flow).

Since this only applies going forward, the two already-approved repos (`suzuki-sv650-1999`, submitted by hypnolope; `blayde-manual-2026`, submitted by TheBlayde) need their real submitters added as collaborators by hand, to match what the new code would have done automatically.

**Backfilled 2026-08-27, and a second real bug found in the process.** Adding hypnolope and TheBlayde as collaborators via the GitHub API worked, but checking the result surfaced that `GET .../collaborators` (no `affiliation` param) DEFAULTS to `affiliation=all` -- confirmed against GitHub's own docs -- which includes every org member with the org's default repository permission (`read`, for BlaydeManual), not just people with a real, explicit grant on that specific repo. Live evidence: `blayde-manual-2026`'s collaborator list included hypnolope with `pull`-only access, even though she was never added to that repo -- just a BlaydeManual member with default read. `my-vehicles.js`'s `fetchRoster()` would have shown every org member on every vehicle's roster as a phantom "read" maintainer. Fixed by adding `affiliation=direct` to the query; confirmed live afterward that `blayde-manual-2026`'s roster now shows only TheBlayde, the real submitter.

## The push-only-maintainer roster gap, and the Worker-scoped fix (2026-08-27)

Direct report, seen live on hypnolope's own account: opening My Vehicles for `suzuki-sv650-1999` (where she'd just been granted real `push` access by the approval-time auto-grant above) showed a correct, read-only roster with the note "only a repo admin can manage its collaborators" -- true, but it meant the actual intended function (a real maintainer adding other contributors) was unreachable for anyone who isn't also a repo Admin. Direct question raised: is there real risk in just granting repo Admin to fix this, or can the role be scoped down?

**Researched rather than guessed, against GitHub's own repository-roles docs**: there is no scoped-down option. Managing collaborators is Admin-only; `Maintain`, one level below Admin, explicitly does not include it. So the only two real choices were "grant real Admin" or "don't rely on GitHub's own role system for this action at all."

**Real Admin's actual cost, stated precisely, not just "it's more permission":** delete the repo, transfer it out of the org, flip it back private, rename it (which would silently break `registry.json`'s `repo_url` pointer -- nothing re-syncs that automatically). Scoped to one repo at a time (GitHub Admin doesn't cascade beyond the repo it's granted on), but real, and more than "let a maintainer invite a contributor" needs.

**The fix, matching the project's own established pattern rather than inventing a new one**: a new privileged Worker endpoint, `POST /manage-collaborator`, using the installation token the same way `/approve-vehicle` and `/direct-contribute` already do. It never trusts the browser's claimed permission -- it independently re-checks the caller's real permission on that specific repo (`GET .../collaborators/{login}/permission`, requiring `admin`/`maintain`/`write`) before doing anything, and any invite it performs always grants `push`, never higher, matching the same floor as the automatic grant on approval. `my-vehicles.js`'s client-side gate (previously `permissions.admin`) is gone entirely -- everything in `discoverMaintainedRepos()`'s result already passed the push-or-better filter this endpoint itself requires, so there's nothing left to gate on client-side. Listing the roster is unaffected -- GitHub already allows any push-or-better collaborator to list collaborators with their own token, so that call stays direct, unprivileged.

Verified via a synthetic Node test against the actual Worker handler (push-level caller succeeds; read-only caller correctly rejected; a non-registered repo correctly rejected before any permission check) and a synthetic browser test of the real `my-vehicles.js` UI (a push-only, non-admin repo now shows the real invite controls and correctly calls the new endpoint).

Answered directly, since it was asked: no, none of this app's real functions (merging photo PRs, editing `manifest.json`, now roster management via the Worker) need anything `Maintain` adds beyond `Write` -- the existing automatic grant on approval already grants `push`, correctly, no change needed there.

## Real photo content validation, real dual-approval, and the Worker deploying itself (2026-08-27)

Direct question, raised after the maintainer-roster work above: "how do we ensure someone doesn't load up garbage in our repos?" -- flagged explicitly as a real but low-severity concern ("not huge... just want to keep it clean"), and paired with a second question about whether quorum-style dual-approval should extend to photo requests too, with an explicit "we can create more accounts" acceptance of whatever that requires operationally.

**Quorum on vehicle approval itself**: left deferred, per direct instruction, until a second real org admin exists -- not decided against, just not yet relevant with only one admin today.

**Real image content validation, `/direct-contribute`.** Previously only checked that `photo_data_url` LOOKED like a data URL (`data:image/jpeg;base64,...` shape) -- no check the content was actually a valid, real image. Added real, dependency-free validation: magic-byte checks per format (JPEG/PNG/WEBP), and real dimensions extracted by parsing each format's own header (PNG's fixed-offset IHDR chunk; JPEG's SOF marker, scanned; WEBP's VP8/VP8L/VP8X sub-formats, each encoding dimensions differently) -- no full pixel decode, no library dependency. Rejects corrupt/near-empty/absurdly-sized uploads before a branch is ever created on the upstream repo, checked before any GitHub call happens. **Deliberately no minimum file-size check**, despite that being an obvious first instinct: built one initially, then found live (via real generated test fixtures, not just theory) that a validly-encoded 800x600 WEBP compressed to under 1KB -- a flat-color PNG did too. Byte size varies too much by content and format to be a reliable "too small" signal; the real floor is the dimension check (200x200 minimum), which a genuinely corrupt or degenerate file fails anyway regardless of its size. Verified via a synthetic Node test against the real Worker handler: a real photo passes and reaches the actual GitHub branch-creation call; a garbage file and a 1x1-pixel file are both rejected before any GitHub mutation happens at all.

**Real dual-approval on photo contributions, enforced by GitHub itself.** `POST /approve-vehicle` now sets real branch protection (`required_approving_review_count: 2`) on a vehicle's default branch at the moment of approval. Deliberately NOT app-level logic inside `review-panel.js` -- directly connects to the same insight from the earlier "can't a maintainer just merge directly via git" conversation: a maintainer with real push access can always bypass anything this app's own UI checks, so an app-level "requires 2 approvals" would be exactly as bypassable as no check at all. Branch protection is GitHub's own enforcement, the one version that can't be. `enforce_admins` stays `false`, matching the same deliberate escape-hatch decision already made for the main tooling repos.

**Real, immediate operational consequence, stated plainly rather than left implicit**: a vehicle repo with only one real maintainer -- which is every vehicle, immediately after approval, since the automatic maintainer-grant just created its first -- cannot merge ANY photo PR until a second real maintainer is added. Intentional (one person approving their own photo isn't dual anything), not a bug, but it means a freshly-approved vehicle has zero contribution throughput until staffed. Applied going forward only, NOT retroactively to the two already-approved vehicles (`suzuki-sv650-1999`, `blayde-manual-2026`) -- both currently have exactly one real maintainer each, so turning this on for them right now would immediately freeze their existing, working review flow. Both need a second real maintainer added first (matching the "we can create more accounts" plan) before branch protection gets turned on for them by hand. Verified via a synthetic Node test against the real Worker handler: the branch-protection PUT call fires with the exact expected payload during a real approval.

**The Worker now deploys itself.** `.github/workflows/deploy-worker.yml` runs `wrangler deploy` on every push to `main` that touches `auth-worker/`, closing a chore this project flagged as a "recurring manual chore" waiting to happen back when the User-Agent bug first needed a manual dashboard-paste redeploy. One-time setup only: a `CLOUDFLARE_API_TOKEN` (Cloudflare dashboard, "Edit Cloudflare Workers" template) and `CLOUDFLARE_ACCOUNT_ID`, both as GitHub Actions repository secrets -- distinct from the Worker's own five Cloudflare secrets (`GITHUB_APP_PRIVATE_KEY` etc.), which still work exactly as before. Manual `wrangler deploy` from a local checkout still works as a fallback (e.g. testing before opening a PR), just isn't the normal path for a real change anymore.
## Two real, live bugs found testing the main page: fingerprint matching, and Browse (2026-08-27)

Direct report: "Choose File" on the main page wasn't finding the registry for either real approved vehicle, and Browse "is not correct either." Both real, both root-caused against actual code and live data, not assumed.

**Fingerprint matching, root cause.** `registry.json`'s `source_pdf_sha256` field -- what the main page's Choose File flow fingerprints an uploaded PDF against (`findByFingerprint()` in `registry.js`) -- was being written at approval time from `logEntry.manifest_sha256`: the MANIFEST's own hash (used for notarization/tamper-detection), a completely different value from the actual source PDF's hash. Confirmed live: recomputed the real sha256 of the actual `blayde-manual-2026` source PDF directly, and it did not match what was stored. The two hashes could never have matched, for any vehicle, ever -- this wasn't a corner case.

**Fix, two parts.** (1) `indexer-ui.js` now sets `manifest.source_pdf_sha256 = selectedPdfHash` right after indexing finishes -- `selectedPdfHash` is the exact same SHA-256 already computed at file-selection time for the local "already registered?" pre-check, just never previously threaded into the submitted manifest at all. (2) `handleApproveVehicle` now writes `manifest.source_pdf_sha256` into the registry entry instead of `logEntry.manifest_sha256`, with a `vehicle_class` field added alongside it (present in every real manifest already, via the submit form's required field, but never previously copied into the registry entry -- needed for Browse's real fix below).

**Live data corrected directly, not just the code going forward.** Both already-approved vehicles had wrong registry entries with no way to self-heal from a code fix alone. `blayde-manual-2026`: recomputed its real hash directly from the exact source PDF file used to submit it (`shasum -a 256`), pushed a corrected `registry.json` entry. `suzuki-sv650-1999`: its manifest's `source_markers.source_identifier` is a real, direct download URL (manualslib.com) -- re-downloaded the full 74MB file from that URL and computed its real hash the same way. Caveat stated plainly: this assumes the file at that URL today is byte-identical to what hypnolope actually uploaded; a reasonable assumption (a static file at a fixed path) but not independently confirmed against her original local copy. `vehicle_class` (`motorcycle` for both, read from each repo's real manifest.json) added to both entries at the same time. Verified live afterward: `findByFingerprint()` against the real, corrected registry now resolves `blayde-manual-2026`'s real PDF hash to the real vehicle record.

**Browse, root cause.** `registry-browse.js` was never wired to real data at all -- confirmed by reading it directly: `MOCK_REGISTRY_BROWSE`, four entirely fabricated vehicles (a Suzuki SV650 with fake stats, a Kawasaki, a Yamaha), untouched since it was first built as a placeholder (its own header comment already said as much). Not a regression, a known gap finally being addressed.

**Fix, and one deliberate simplification.** Rewrote it to fetch the real `registry.json`, and for each real vehicle, real per-edition stats computed by fetching the actual `manifest.json` and the actual `images/` directory listing (reusing `registry.js`'s existing `fetchManifest`/`listRepoImages`/`parsePhotoFilename` -- the same real mechanism the patcher itself already relies on for "does this procedure have a contributed photo," which is a real file's presence in `images/`, NOT the manifest entry's own `status` field, confirmed by checking: that field is written once at indexing time and never updated by anything in this codebase afterward). **Dropped**: grouping multiple generations of the same make+model under one heading, and filtering/sorting by make/model/year_range -- `registry.json` has no such fields today, only `vehicle_slug`/`vehicle_display_name`/`vehicle_class`/`edition_id`/`repo_url`/`source_pdf_sha256`/`status`. Faking a make/model split via slug-parsing guesswork would be worse than not having it; reintroducing that grouping for real needs those fields captured at submit time first, not built here as a workaround. What's kept, because it IS backed by real data: one card per real `vehicle_slug`, with multiple real editions listed under it (registry.json already supports several entries sharing one slug, via org-approval's "add edition" path).

Verified live against production: both real vehicles now render on Browse with real edition IDs, real source links, and real procedure counts (6 for `blayde-manual-2026`, 918 for `suzuki-sv650-1999`, both matching their actual manifests exactly), 0% coverage (correctly -- no photos have actually been contributed to either yet), and the real `vehicle_class` populating the type filter.

## In-PDF QR marker and credit tab: real bottom-right placement, a real clickable link, and PDF layers as a deliberately-declined option (2026-08-27)

Direct feedback on the patched output: reposition the missing-photo QR to the bottom-right of its own box (not centered/top), make it a real clickable link (not just scannable), and encode contributor credit visually on a contributed photo's own bottom-right corner -- explicitly NOT baked into the stored photo file itself, since style/placement may change later and nobody should have to re-upload anything to pick that up. Confirmed: the credit tab and QR marker are both drawn fresh by `patcher.js` on every patch run, computed from data already available at patch time (the contributor's GitHub handle from the photo's own filename convention; the contribute URL from `repo_url`/`procedure_id`) -- never written into the photo bytes themselves.

**QR marker (`drawContributeMarker` in `web/patcher.js`)**: QR image moved to the bottom-right corner of its own placeholder box (previously centered, near the top). The caption ("photo needed...") stays top-left; the URL is now also printed as plain text bottom-left of the QR, for anyone reading on paper without a scanner. Real clickable link added over the whole box via a PDF Link annotation (`/Subtype /Link`, `/A << /S /URI /URI (...) >>`), so tapping anywhere in the dashed box in a real PDF viewer opens the contribute page directly, not just scanning a second device.

**A verification false alarm worth recording.** The link annotation initially looked broken: constructing it via `page.doc.context.obj([...])` (after finding `PDFArray.of()` doesn't exist in this pdf-lib fork) threw no error, but a raw text/regex search over the saved PDF's bytes found zero occurrences of `/Subtype /Link`, `/URI`, or the test URL anywhere. Root-caused with a standalone Node reproduction outside the browser (`@cantoo/pdf-lib` installed directly, no DOM): the annotation dict was really being written, correctly referenced from the page's `/Annots` array (confirmed via `page.node.get()` right after construction) -- it just gets compressed into a Flate-encoded object stream (`/Type /ObjStm`) by pdf-lib's default save behavior, which a plain text search over the raw bytes can never see. Confirmed real (not just "no longer erroring") by reloading the saved PDF back through `PDFDocument.load()` and resolving the `/Annots` ref to the actual `/URI` string. Lesson for this codebase specifically: verifying pdf-lib output by raw-text-searching saved bytes is unreliable for anything pdf-lib chooses to object-stream-compress -- reload-and-resolve is the only verification that actually proves persistence.

A long, unbroken URL string can't word-wrap the way the caption text does (`drawText`'s `maxWidth` only wraps at spaces), so an early version overlapped the bottom-left URL text under the QR image. Fixed the same way the credit tab already handles a long handle: measure the real string width at the target font size and shrink the font to fit the available space, rather than relying on `maxWidth`.

**Credit tab (`drawCreditTab`)**: first pass was a diagonal ribbon rotated across the photo's corner -- direct pushback, with a reference image: these are instructional photos, and a diagonal cut reaches well past the literal corner, risking covering the actual subject matter that's the whole point of the photo. Rebuilt as a flat, non-rotated horizontal tag flush in the bottom-right corner only -- sized to the measured width of `@handle` (capped at 60% of the photo's own width, shrinking the font further only if even that cap can't fit the text), so it only ever touches the corner it's anchored to.

**PDF layers (Optional Content Groups), asked about directly, declined as an option for this project.** Real, standard PDF feature since 1.5 -- pdf-lib can mark content with `/OC` and register groups via the catalog's `/OCProperties`. The real problem: almost no viewer outside Acrobat/Acrobat Reader exposes a toggle UI for it -- not Chrome's or Firefox's built-in viewers, not macOS Preview, not the default viewers on iOS/Android. Given this project's own stated use case for the QR feature is specifically "someone on their phone, mid-repair, not at a desk," a feature that only works in one desktop app isn't a real option here. Could still be useful later for a hidden, never-toggled-by-a-reader fallback layer (e.g. archival), but not as an interactive show/hide control -- noted for if the "see the original photo before it was replaced" idea below ever gets built for real.

**Same "not actually controllable from the PDF" flag as the layers item above, this time on the contribute link itself, raised 2026-09-02.** Real complaint: clicking the QR box's link annotation opens the contribute page in the SAME window/tab as the manual, losing your place in the document -- annoying enough to be worth a fix, not urgent enough to block anything. Checked directly: standard PDF `/Link` annotations have no equivalent to HTML's `target="_blank"` -- there's no "open in a new window" flag in the spec for a `/URI` action, so this is entirely up to whichever PDF viewer renders the file, not something `addLinkAnnotation` (or anything else in `patcher.js`) can force. Not pursued further -- nothing to build here until/unless a real viewer-side workaround surfaces.
...
## QR payload shortened to a vehicle slug; two more shrink levers, backlogged (2026-08-27)

Following the credit-tab/QR work above: every QR in one patched manual pointed at the same vehicle, but `contributeUrl()` was encoding the vehicle's full GitHub repo URL into every single one. Changed to encode `vehicle_slug` instead (`contribute.html?v=<slug>&procedure=<id>`), with `contribute.js` resolving it back to a real `repo_url` via `registry.json` -- the same registry lookup `registry-browse.js` already does, just for one slug instead of the whole list. A `repo=<full URL>` param (legacy) still works directly and un-resolved, so a QR already printed into someone's patched manual before this change doesn't break. Measured on a real live procedure (`blayde-manual-2026`, `p003_proc1_fig1`): 132 chars / 49x49 modules -> 87 chars / 41x41 modules. Verified live against both param shapes on the real production registry, not mocked.

**Two more shrink levers, named but explicitly not built -- direct instruction to backlog only, since `vehicle_slug` already solves today's real vehicles:**
1. **A real short/opaque ID field on the registry entry**, e.g. a small integer or short code assigned at approval time, used instead of `vehicle_slug` in the QR when present (same resolution mechanism, just matching on `id` instead of `vehicle_slug`). The real motivating case: `vehicle_slug` is only short because today's two live vehicles happen to have short names -- a real make like "Alfa Romeo Giulietta Quadrifoglio" would produce a much longer slug, undoing most of the win above. An opaque ID sidesteps that entirely regardless of how long the real vehicle name is.
2. Shortening the encoded string further with a real redirect/short-link layer was considered and explicitly rejected in the same conversation as not worth the added infrastructure (a new backend concern) when a same-registry ID field gets most of the same benefit for free.

Not scoped further than this until vehicle names longer than today's actually show up in the registry.

## Registry repo's schema doc has real drift from what the code actually writes (2026-08-27)

Direct report while reviewing the QR/slug change above: the registry repo's (`BlaydeManual/registry`, a separate repo from this one -- needs its own PR there, not here) `README.md` schema example still shows `"vehicle_slug": "make-model-year-range"`. Checked against the real, live `registry.json` in that repo and confirmed more drift than just that one field: neither live entry's slug actually follows a `make-model-year-range` shape (both are just `suzuki-sv650-1999` / `blayde-manual-2026`); the documented example also shows `source_identifier` and `submitted_by` fields that no real entry has, and is missing `vehicle_class`, which every real entry does have and which Browse's type-grouping and the approval flow both depend on. Not fixed here -- flagged for a separate PR against `BlaydeManual/registry` directly, since this repo (`blayde-manual`) has no write access to that one's docs as part of this branch.
...
## Contributor Portal redesign: only "Save for Review," no direct submit; sign-in deferred to actual Submit (2026-08-30)

Direct correction to the original flow: "I actually only want them to save in their drafts. Nobody should submit anywhere directly. You should have the opportunity to view it in the reader and make sure it's right and you feel good about it, and that you picked the correct picture." The capture screen ("Propose a photo for this procedure") no longer has a "Submit now" button or a Public/Private choice at all -- **Save for Review** is the only action, and it always just writes to this device's own localStorage. The Public/Private choice, and the sign-in it requires, both moved to the moment someone actually clicks Submit from **My Reviewables** (renamed from "My uploads," reusing the project owner's own word for what these are) -- the one place a maintainer or contributor reviews the photo side-by-side against the original page before anything becomes real.
...
**Shipped, 2026-08-31** ([blayde-manual#75](https://github.com/BlaydeManual/blayde-manual/pull/75)): the Worker-side merge-gate (`handleAcceptRecategorization`, `POST /accept-recategorization`) -- registry.json-only file-allowlist + single-entry diff validation, mirroring the existing photo-PR/vehicle-approval gates. **Still not built:** the actual UI that lets a contributor generate one of these PRs in the first place (a "propose recategorization" action) -- today the gate only has something to approve if someone opens a PR against the registry repo by hand.
...
**Expanded, 2026-08-31: what happens when someone picks a fuzzy match anyway.** Direct instruction, folded into the same not-yet-built future path above rather than treated as a separate feature: the "did you mean one of these" prompt isn't just a fork between "yes, that's mine" (stop, go straight to that existing vehicle) and "no" (fall through to full indexing as new). A real third case exists -- the submitter believes it's genuinely a different edition/printing (a re-scan, a slightly different revision, a different source) but the fuzzy match still flagged real textual similarity worth not just discarding. For that case: **don't silently create a second, disconnected registry entry, and don't force a full duplicate indexing pass either.** Offer to open an issue/PR proposing this new PDF's fingerprint be added to the *existing* matched entry as an **optional/alternate accepted fingerprint** (`source_pdf_sha256` becoming a list, or a new `alternate_fingerprints` array alongside it -- exact schema is a real decision for whenever this gets built, not decided here) rather than recreating the whole vehicle/edition from scratch. A human reviewer (same org-level quorum as any other registry.json edit, per the recategorization flow above) confirms it's actually the same underlying manual before merging. This reuses the existing entry's manifest and already-contributed photos instead of starting a second, competing indexing project for what's likely the same real-world document with different bytes. Not designed further than this paragraph -- logged here so the idea isn't lost, to pick up once the fuzzy-match resolution path itself is actually being built.
...
**Shipped:**
1. Indexer's "confirm the vehicle" step's Category + Manual Type dropdowns ([blayde-manual#72](https://github.com/BlaydeManual/blayde-manual/pull/72)).
2. `registry-browse.js`'s category-tab UI ([blayde-manual#73](https://github.com/BlaydeManual/blayde-manual/pull/73)).
3. Maintainer/Contributor category grouping tier ([blayde-manual#74](https://github.com/BlaydeManual/blayde-manual/pull/74)).
4. The `registry.json`-only recategorization merge-gate, Worker-side (`handleAcceptRecategorization`, `POST /accept-recategorization` in `auth-worker/src/index.js`) -- mirrors `handleAcceptPhotoPr`'s negative-allowlist shape and `handleApproveVehicle`'s org-approver bar + `dry_run` pattern. Checks: exactly one file changed (`registry.json`, `modified`, never added/removed); no vehicle/edition entries added, removed, or re-identified between the PR's base and head commits; exactly one entry actually differs; every differing field on it is `category` and/or `manual_type`, nothing else; the resulting pair is real per `manual-types.json`'s current default-branch content. Merge is pinned to `pr.head.sha`, same TOCTOU protection as the photo-PR gate. **Not built yet, and not this item's scope:** the actual UI that lets a contributor generate one of these PRs in the first place (a "propose recategorization" action) -- today this only gates a PR opened by hand directly against the registry repo, same as a photo PR could always be opened directly via git before contribute.js existed. **Not tested end-to-end**: no local test harness exists for this Worker (no `package.json`, no test runner) and exercising it for real needs an actual open PR against the registry repo plus provisioned GitHub App credentials -- verified via `node --check` and careful mirroring of the two existing gates' already-proven shape, not a real live run. Recommend one real live test (open a hand-crafted recategorization PR, dry-run it, then approve it for real) once there's a real "other" entry worth recategorizing.
5. ~~Mosaic template key changes from a flat `vehicle_class` to `category/manual_type`.~~ **Corrected, 2026-08-31: this item assumed a template system that doesn't exist.** Checked `mosaic.py` directly rather than trusting this file's own prior wording -- there is no `templates/` directory, no per-`vehicle_class` (or any) dispatch, and nothing else in the codebase even calls `build_mosaic()` yet; it's a standalone script hardcoding exactly one hand-drawn silhouette (a motorcycle, with `ZONES`/`ZONE_KEYWORDS` tuned to motorcycle service terms) and isn't wired into the patcher pipeline at all. "Rekey" was never the real task -- there was nothing keyed by `vehicle_class` to rekey. **Direct instruction: leave `mosaic.py` fully untouched for now** -- a real per-category/manual_type template system (which silhouettes exist, how zones/keywords generalize past a motorcycle, whether every manual type even needs one vs. a generic outline fallback) is real design work deserving its own dedicated pass, not a quick fix bolted onto this item's original wrong premise. Tracked here as: disconnected from the category/manual_type system entirely, current single-motorcycle behavior unaffected either way, revisit wholesale when it's actually prioritized.

**Scope call made while building item 2:** the public main page's (`index.html`) illustrated hero -- a fully custom before/after diagram built around one specific real repair manual's actual photos -- stays as-is rather than getting six fabricated category-specific photo sets. **Direct instruction, 2026-08-31: this gets a dedicated focused pass separately**, once the rest of this category-expansion work is done -- not folded into this same session's remaining scope, and not treated as indefinitely parked like the mosaic item above.
...
## Photo-location-fix proposals: what shipped, and two items logged, not built (2026-09-02)

**Shipped**: any signed-in contributor (not just a manual's own maintainers) can propose a box reposition, a missed photo slot, or flag a tracked slot for removal, via the Contributor Portal's "Propose a photo location fix" -- a real fork+PR reviewed through `/accept-manifest-change`, same "requesting is Contributor, reviewing is Maintainer" rule the rest of this site follows. The Maintainer Portal's "Review Photo Requests" shows these alongside photo requests, with a dedicated full-page color-coded diff view (add = blue, remove = blue with a corner-to-corner X, reposition = current position in green next to proposed in blue). See SECURITY-TESTING.md's Tier 8.

**Deliberately dropped, not deferred:** the original design also included a "comment" issue kind (a note with no bbox, queued via right-click "Add a comment" on an existing photo). No real use case ever surfaced to justify building it for real -- rather than ship a half-real feature, it's removed from the UI entirely. Revisit only if a real, specific need for it shows up.

**Backlogged, not started:** proposing a correction to a manual's own registry metadata that isn't category/manual_type -- specifically its source URL, if that ever goes stale or wrong. Flagged directly as likely the same shape as this feature (Contributor proposes, Maintainer reviews, a narrow single-field diff gate), but explicitly parked to avoid scope creep while this pass was still in progress. Pick up once there's a real, specific need.
