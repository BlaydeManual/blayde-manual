...
## Backlog: two more QR-payload shrink levers, beyond the vehicle-slug shortening already shipped

**Not built, since `vehicle_slug` already solves today's real vehicles:**
...
See CHANGELOG.md for the shipped data foundation, dropdowns, category tabs, grouping tier, and recategorization merge-gate. Still open:

- **A UI for a contributor to actually open a recategorization PR.** The Worker-side gate only has something to approve once a PR exists by hand against the registry repo.
- **A per-category mosaic template system.** `mosaic.py` has no `templates/` directory or per-category dispatch -- it's a standalone script hardcoding one motorcycle silhouette, not wired into the patcher pipeline at all. Left fully untouched; a real system (which silhouettes exist, how zones/keywords generalize, whether every manual type needs one vs. a generic outline) is its own dedicated design pass.
- **The public main page's illustrated hero** stays a single custom motorcycle before/after diagram rather than getting six category-specific photo sets -- planned as its own dedicated pass once the rest of this work settles.
- **The recategorization gate's live end-to-end test** is still synthetic-only (verified via `node --check` and mirroring the two existing gates' proven shape) -- needs one real live pass (hand-craft a recategorization PR, dry-run it, approve it for real) once there's an `other` entry worth recategorizing.
...
## Photo-location-fix proposals: two items logged, not built (2026-09-02)

Photo-location-fix proposals shipped -- see CHANGELOG.md (PR #92) and SECURITY-TESTING.md's Tier 8.
