...
**Shipped** ([blayde-manual#72](https://github.com/BlaydeManual/blayde-manual/pull/72)): indexer's "confirm the vehicle" step's Category + Manual Type dropdowns (item 1 below).

**Not yet built, real next steps, roughly in dependency order:**
1. ~~Indexer's "confirm the vehicle" step gains Category + Manual Type dropdowns, populated from `manual-types.json`, validated before submit (the actual point where free text would otherwise leak in).~~ Shipped, see above.
2. `registry-browse.js` gains the category-tab UI (hard filter there, unlike the maintainer/contributor side -- see below): Browse results and search scoped to the active tab, secondary Manual Type filter cascading from it, default "All" (not a forced choice on first visit). **Scope call made while building this:** the public main page's (`index.html`) illustrated hero -- a fully custom before/after diagram built around one specific real repair manual's actual photos -- stays as-is rather than getting six fabricated category-specific photo sets. Revisit only with real category-specific source imagery in hand, same "don't build it speculatively" posture as the mosaic template rekey below.
