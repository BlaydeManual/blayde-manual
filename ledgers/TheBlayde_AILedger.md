...
## Branch-protection hardening, a stale-plan mistake, and a documentation cleanup pass

**The security hardening itself was mostly Claude-executed once directed.** After the user asked how a maintainer could fake their own activity stats, Claude audited branch protection across every repo directly against the live GitHub API rather than trusting any doc's claim, found `royal-lexon-s20` had zero protection and was missing its entire CI scaffold, and found `submission-log` (the App-only notarization log) unprotected too. The user approved the fix plan, and Claude applied it: scaffold files added via PR, protection matched to the existing 2-reviewer standard, and a tamper-lockdown shape on `submission-log` (`enforce_admins: true`, force-push/deletion blocked, restricted to the App's own installation) that didn't exist as a pattern anywhere else in the project until this session.

**A real Claude mistake, caught by a direct user question, not Claude's own review:** asked to register a new GitHub App per an old local plan file, Claude gave the full registration walkthrough twice before the user asked "why do we need to do this again? can we not reuse the same app we built?" The App (`blayde-manual-direct`) was already registered, installed, and live in production weeks earlier -- fully documented in ROADMAP.md's own history, which Claude had access to but didn't check against the stale plan file before repeating its instructions. The user's question, not Claude's own diligence, surfaced that the plan file was pure dead weight describing already-shipped work.

**The ROADMAP.md cleanup that followed had two rounds of the same correction.** Claude's first attempt at closing a resolved backlog item marked it "Closed, 2026-09-10" in place in ROADMAP.md -- directly contradicting a rule already on record from an earlier session ("roadmap is what's coming, not what's built"). The user caught it immediately a second time: "don't put closed things in roadmap, that's changelog and roadmap SHRINKS." Claude then delegated a much larger cleanup pass (the user had personally reviewed and given verdicts on ~35 sections) to a background agent with the corrected rule spelled out explicitly. That pass cut ROADMAP.md by two-thirds and was mostly right -- but the user asked a plain verification question afterward ("Does 124 change Security.md? I don't see that one touched") that surfaced a real gap: the agent had deleted a section containing a genuinely still-open item (missing CI workflow `permissions:` blocks) and a real, live, shipped feature (the Contributor Portal's Save-for-Review redesign, PR #59) with zero replacement in either SECURITY.md or CHANGELOG.md. Both were real content losses from an automated pass Claude had already reviewed once and signed off on as correct. Fixed once asked to look again, closely, at the full diff rather than a spot-check.
